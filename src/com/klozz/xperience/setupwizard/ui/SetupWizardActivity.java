...
package com.klozz.xperience.setupwizard.ui;

import android.accounts.*;
import com.klozz.xperience.setupwizard.XPeSetupWizard;
import com.klozz.xperience.setupwizard.R;
import com.klozz.xperience.setupwizard.gcm.GCMUtil;
import com.klozz.xperience.setupwizard.setup.AbstractSetupData;
import com.klozz.xperience.setupwizard.setup.XPeSetupWizardData;
import com.klozz.xperience.setupwizard.setup.Page;
import com.klozz.xperience.setupwizard.setup.PageList;
import com.klozz.xperience.setupwizard.setup.SetupDataCallbacks;
import com.klozz.xperience.setupwizard.util.XPeAccountUtils;
import com.klozz.xperience.setupwizard.util.EnableAccessibilityController;
...
public class SetupWizardActivity extends Activity implements SetupDataCallbacks {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ((XPeSetupWizard) AppGlobals.getInitialApplication()).disableStatusBar();
        mSharedPreferences = getSharedPreferences(XPeSetupWizard.SETTINGS_PREFERENCES, Context.MODE_PRIVATE);
        mSetupData = (AbstractSetupData)getLastNonConfigurationInstance();
        if (mSetupData == null) {
            mSetupData = new XPeSetupWizardData(this);
        }
    ...
    protected void onResume() {
        super.onResume();
        onPageTreeChanged();
        if (!XPeAccountUtils.isNetworkConnected(this)&& mTriedEnablingWifiOnce) {
            XPeAccountUtils.tryEnablingWifi(this);
        }
    }
                    ...
                    switch (page.getId()) {
                        case R.string.setup_welcome:
                            if (!mTriedEnablingWifiOnce) {
                                ...
                                XPeAccountUtils.launchWifiSetup(SetupWizardActivity.this);
                            }
                            break;
    ...
    private void removeUnNeededPages() {
        ...
        if (page != null && (!GCMUtil.googleServicesExist(SetupWizardActivity.this) || accountExists(XPeSetupWizard.ACCOUNT_TYPE_GOOGLE))) {
            removeSetupPage(page, false);
            pagesRemoved = true;
        }
        if (pagesRemoved) {
    ...
    public void launchGoogleAccountSetup() {
        ...
        bundle.putBoolean(XPeSetupWizard.EXTRA_FIRST_RUN, true);
        bundle.putBoolean(XPeSetupWizard.EXTRA_ALLOW_SKIP, true);
        AccountManager.get(this).addAccount(XPeSetupWizard.ACCOUNT_TYPE_GOOGLE, null, null, bundle, this, new AccountManagerCallback<Bundle>() {
            @Override
	...
	if (mSetupComplete) return;
        ...
        ((XPeSetupWizard) AppGlobals.getInitialApplication()).enableStatusBar();
